...
def volume_water(trig_beacons):
    """
    :param trig_beacons: list
    :return accum_volume: int
    """
    # Initialise
    abs_max = 0
    volume = 0
    ...
    for height in trig_beacons:
        if height < abs_max:
            # Temp volume counter
            volume = volume + abs_max - height
        else:
            # Set max water level
            abs_max = height
            # At max, add accumulated volume and reset temp count to zero
            accum_volume = accum_volume + volume
            volume = 0
    ...
    for height in reversed(trig_beacons):
        if height < local_max:
            volume = volume + local_max - height
        else:
            local_max = height
            accum_volume = accum_volume + volume
            volume = 0
        # Break when absolute max reached
        if height == abs_max:
            return accum_volume
...
def test():
    # Inside lakes
    test0 = [1, 3, 2, 4, 1, 3, 1, 4, 5, 2, 2, 1, 4, 2, 2]
    answer = volume_water(test0)
    assert answer == 15, \
        "answer was {}, not 15: ".format(answer)

    # One big inside lake
    test1 = [1, 3, 1, 1, 1, 1, 1, 3, 1]
    answer = volume_water(test1)
    assert answer == 10, \
        "answer was {}, not 10: ".format(answer)

    # Max at begin
    test2 = [8, 2, 3, 4, 5, 6, 7]
    answer = volume_water(test2)
    assert answer == 15, \
        "answer was {}, not 15: ".format(answer)

    # Left to right step
    test3 = [1, 2, 3, 4, 5, 6, 7]
    answer = volume_water(test3)
    assert answer == 0, \
        "answer was {}, not 0: ".format(answer)

    # Random set
    test4 = [244, 246, 34, 35, 226, 30, 203, 116, 219, 114, 217, 95, 141, 142,
    ...
    answer = volume_water(test4)
    assert answer == 11960, \
        "answer was {}, not 11960: ".format(answer)

    # small dips
    test5 = [2, 2, 1, 3, 2, 3, 3, 2, 1, 2, 1, 2, 3, 2, 1, 3, 1, 1, 1, 2, 3, 1,
    ...
    answer = volume_water(test5)
    assert answer == 80, \
        "answer was {}, not 2: ".format(answer)

    # negative numbers
    test6 = [0, 1, -2, -2, -2, -2, 0, -2, 0, -2, -2, -1, 1, -2, -1, -1, -2, -2,
             ...
             -2, 1, -2, 1, -1, -3, -2, -3, 1]
    answer = volume_water(test6)
    assert answer == 208, \
        "answer was {}, not 208: ".format(answer)

    # huge set 1028
    test7 = [233, 230, 75, 250, 48, 164, 167, 187, 164, 189, 59, 56, 138, 81,
    ...
    answer = volume_water(test7)
    assert answer == 129507, \
        "answer was {}, not 129507: ".format(answer)
